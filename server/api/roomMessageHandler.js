...
class RoomMessageHandler extends MessageHandler {
  ...
  #handleIncomingCellUpdateMessages(socket) {
	...
	const pieceId = parseInt(pieceIdStr, 10);
      ...
      if (trackMap[pieceId]) {
        try {
          // Log individual placement event
          await this._databaseConnector.executePreparedQuery(
            `INSERT INTO piece_placement_events
                (game_id, player_name, piece_id, placement_time, cell_position)
                VALUES (?, ?, ?, NOW(), ?)`,
            [roomId, playerName, pieceId, position]
          );

          // Update aggregated statistics
          await this._databaseConnector.executePreparedQuery(
            `INSERT INTO player_piece_stats
                (game_id, player_name, piece_id, placement_count, first_placement_time, last_placement_time)
              VALUES (?, ?, ?, 1, NOW(), NOW())
          ...
          console.error("Stats upsert failed:", err);
  ...
  #handleIncomingChatMessages(socket) {
    socket.on("chat msg", (data) => {
      ...
      this.SendChatMessage(socket, roomId, message, name);
    });
  }
  ...
  SendGameOverPacket(socket, roomId, message, name) {
    if (this._rooms[roomId]) {
      ...
      //send a message to all players in the room that a new player has joined.
      //since the socket is now subscribed to the room, it will also receive the message.
      this._io.to(roomId).emit("gameOver", { roomId: roomId });
  ...
  async enterRoom(socket, roomId, playerName) {
    if (this._rooms[roomId]) {
      const players = this._rooms[roomId].players;
      ...
      if (this._rooms[roomId].players.length === 2) {
        ...
        try {
          await this._databaseConnector.executePreparedQuery(
            "INSERT INTO games (game_id, room_id, start_time) VALUES (?, ?, NOW())",
            [roomId, roomId]
          );

          for (const player of this._rooms[roomId].players) {
            await this._databaseConnector.executePreparedQuery(
          ...
          console.error("Failed to create game records:", err);
        }
      ...
      socket.emit("handle error", { reason: "room not found" });
    }
  }
