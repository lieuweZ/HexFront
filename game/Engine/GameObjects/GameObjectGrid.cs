					...
					if (Interactible == 0 || Interactible == 2)
					{
						if (MousePos.X > PosX && MousePos.X < PosX + tileScale && MousePos.Y > PosY && MousePos.Y < PosY + tileScale)
							if (DrawingHelper.InsideHexagon(new Rectangle(PosX, PosY, tileScale, tileScale), MousePos))
							{
								...
								Cell hoveredCell = Get(i, j) as Cell;
								if (hoveredCell?.Obj is PieceObject piece)
								{
									Vector2 textPos = MousePos + new Vector2(10, 10);
									spriteBatch.DrawString(GameEnvironment.AssetManager.Content.Load<SpriteFont>("Fonts/SpriteFont"), piece.HoverText, textPos, Color.White);
